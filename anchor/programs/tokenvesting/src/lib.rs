...
pub struct CreateVestingAccount<'info>{ // Account to initialize the vesting contract.
    #[account(mut)]
...
#[account]
#[derive(InitSpace)]
pub struct VestingAccount{ //Account to store/hold the vesting information/state.
    pub owner: Pubkey,
    pub mint: Pubkey,
    pub treasury_token_account: Pubkey, // Account to hold the tokens.
    #[max_len(50)]
...
#[account]
#[derive(InitSpace)]
pub struct EmployeeAccount{ //Account to store/hold the vesting information/state of Employee.
...
Initialize Vesting COntract.
    1) Both Employers and Employees need to have a vesting account.
Give Employer ability to add emplo}yees.
